...
bool EmotiBit::setSamplingRates(SamplingRates s) {
  ...
  _samplingRates.accelerometer =
      25.f * pow(2.f, ((float)imuSettings.acc_odr -
                       6.f)); // See lookup table in BMI160 datasheet
  _samplingRates.gyroscope =
      25.f * pow(2.f, ((float)imuSettings.gyr_odr -
                       6.f)); // See lookup table in BMI160 datasheet
  _samplingRates.magnetometer =
      25.f * pow(2.f, ((float)imuSettings.mag_odr -
                       6.f)); // See lookup table in BMI160 datasheet
...
void EmotiBit::bmm150GetRegs(uint8_t address, uint8_t *dest, uint16_t len) {
  for (uint16_t i = 0; i < len; i++) {
    BMI160.setRegister(BMI160_MAG_IF_2,
                       address); // tell BMI160 to read BMM150 Address,
                                 // automatically flipping the MAN_OP bit to 1
    delay(BMI160_AUX_COM_DELAY);
    // add poll
    uint8_t p = BMI160.getRegister(BMI160_RA_STATUS);
    delay(BMI160_READ_WRITE_DELAY);
    while (EmotiBit::getBit(p, BMI160_STATUS_MAG_MAN_OP) !=
           0) { // wait for MAN_OP to switch back to 0
...
/*!
 * @brief This internal API reads the trim registers of the BMM150 magnetometer
 and stores
...
uint8_t EmotiBit::setup(String firmwareVariant) {
  ...
  setCpuFrequencyMhz(
      CPU_HZ / 1000000); // 80MHz has been tested working to save battery life
#endif
  ...
  while (Serial.available()) {
    ...
    if (input == 'R') {
      ...
      EmotiBitFactoryTest::updateOutputString(
          ackString, EmotiBitFactoryTest::TypeTag::FIRMWARE_VERSION,
      ...
      while (Serial.available() || !barcodeReceived) {
        ...
        if (input == EmotiBitFactoryTest::MSG_START_CHAR) {
          String msg =
              Serial.readStringUntil(EmotiBitFactoryTest::MSG_TERM_CHAR);
          Serial.print("Barcode msg: ");
          Serial.println(msg);
          String msgTypeTag = msg.substring(0, 2);
          if (msgTypeTag.equals(
                  EmotiBitFactoryTest::TypeTag::EMOTIBIT_BARCODE)) {
            ...
            Serial.println(
                "Barcode not received in the correct packet format.");
          }
        }
  ...
  // Added initPinMapping(UNKOWN) to perform basic pin measurements before
  // isEmotiBitReady is successful
  emotiBitVersionController.initPinMapping(
      EmotiBitVersionController::EmotiBitVersion::UNKNOWN);
  ...
  if (emotiBitVersionController.isEmotiBitReady()) {
    ...
    if (testingMode == TestingMode::FACTORY_TEST) {
      EmotiBitFactoryTest::updateOutputString(
          factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::EMOTIBIT_READY,
          EmotiBitFactoryTest::TypeTag::TEST_PASS);
    }
  } else {
    if (testingMode == TestingMode::FACTORY_TEST) {
      EmotiBitFactoryTest::updateOutputString(
          factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::EMOTIBIT_READY,
          EmotiBitFactoryTest::TypeTag::TEST_FAIL);
      EmotiBitFactoryTest::updateOutputString(
          factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::SETUP_COMPLETE,
          EmotiBitFactoryTest::TypeTag::TEST_FAIL);
      Serial.println(factoryTestSerialOutput);
    }
#ifdef ADAFRUIT_FEATHER_M0
    PORT->Group[PORTA].PINCFG[17].bit.DRVSTR =
        1; // Increase SCL pin drive strength to over-power current pulling up
#endif
    // Set Feather LED LOW
    ...
    setupFailed("SD-Card not detected",
                emotiBitVersionController.getAssignedPin(
                    EmotiBitPinName::EMOTIBIT_BUTTON));
  }
  bool status = true;
  ...
  _EmotiBit_i2c =
      new TwoWire(&sercom1, EmotiBitVersionController::EMOTIBIT_I2C_DAT_PIN,
                  EmotiBitVersionController::EMOTIBIT_I2C_CLK_PIN);
  _EmotiBit_i2c->begin();
  ...
  status =
      _EmotiBit_i2c->begin(EmotiBitVersionController::EMOTIBIT_I2C_DAT_PIN,
                           EmotiBitVersionController::EMOTIBIT_I2C_CLK_PIN);
  if (status) {
  ...
  if (testingMode == TestingMode::FACTORY_TEST) {
    EmotiBitFactoryTest::updateOutputString(
        factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::I2C_COMM_INIT,
        EmotiBitFactoryTest::TypeTag::TEST_PASS);
  }
  ...
  if (_emotibitNvmController.init(*_EmotiBit_i2c)) {
    ...
    if (testingMode == TestingMode::FACTORY_TEST) {
      EmotiBitFactoryTest::updateOutputString(
          factoryTestSerialOutput,
          EmotiBitFactoryTest::TypeTag::EMOTIBIT_STORAGE,
          EmotiBitFactoryTest::TypeTag::TEST_PASS);
    }
  } else {
    Serial.println("fail");
  ...
  if (testingMode == TestingMode::FACTORY_TEST && barcode.rawCode != "") {
    ...
    if (emotiBitVersionController.validateBarcodeInfo(
            *(_EmotiBit_i2c), barcode, hwValidation, skuValidation)) {
      EmotiBitFactoryTest::updateOutputString(
          factoryTestSerialOutput,
          EmotiBitFactoryTest::TypeTag::VERSION_VALIDATION,
          EmotiBitFactoryTest::TypeTag::TEST_PASS);
      EmotiBitFactoryTest::updateOutputString(
          factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::SKU_VALIDATION,
          EmotiBitFactoryTest::TypeTag::TEST_PASS);
      if (!emotiBitVersionController.writeVariantInfoToNvm(
              _emotibitNvmController, barcode)) {
        sleep(false);
      }
    } else {
      if (hwValidation) {
      ...
      if (skuValidation) {
        EmotiBitFactoryTest::updateOutputString(
            factoryTestSerialOutput,
            EmotiBitFactoryTest::TypeTag::SKU_VALIDATION,
            EmotiBitFactoryTest::TypeTag::TEST_PASS);
      } else {
        EmotiBitFactoryTest::updateOutputString(
            factoryTestSerialOutput,
  ...
  if (!emotiBitVersionController.getEmotiBitVariantInfo(
          _emotibitNvmController, _hwVersion, emotiBitSku, emotibitSerialNumber,
          emotibitDeviceId)) {
    if (!emotiBitVersionController.detectVariantFromHardware(
            *(_EmotiBit_i2c), _hwVersion, emotiBitSku)) {
      setupFailed("CANNOT IDENTIFY HARDWARE");
    }
  }
  // device ID for V3 and lower will be updated after Temp/Humidity sensor is
  // setup below
  ...
  if (testingMode == TestingMode::FACTORY_TEST) {
    EmotiBitFactoryTest::updateOutputString(
        factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::FIRMWARE_VERSION,
        firmware_version.c_str());
    EmotiBitFactoryTest::updateOutputString(
        factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::EMOTIBIT_VERSION,
        EmotiBitVersionController::getHardwareVersion(_hwVersion));
    EmotiBitFactoryTest::updateOutputString(
        factoryTestSerialOutput,
        EmotiBitFactoryTest::TypeTag::EMOTIBIT_SERIAL_NUMBER,
        String(emotibitSerialNumber).c_str());
    EmotiBitFactoryTest::updateOutputString(
        factoryTestSerialOutput,
        EmotiBitFactoryTest::TypeTag::EMOTIBIT_SKU_TYPE, emotiBitSku.c_str());
  }
  // Serial.println("All Serial inputs must be used with **No Line Ending**
  // option from the serial monitor");
  ...
  emotiBitVersionController.initPinMapping(
      _hwVersion); // Any unknown version is handled in the version detection
                   // code.
  initResult = emotiBitVersionController.initConstantMapping(_hwVersion);

  // Constant Mapping fails if NUM_CONSTANTS not updated in versionController
  // Class
  if (!initResult) {
    Serial.println("Constant Mapping Failed. Stopping execution");
    emotiBitVersionController.initConstantMapping(
        EmotiBitVersionController::EmotiBitVersion::
            V03B); // Assume the version is V03B to set Hibernate level as
                   // Required
  ...
  _batteryReadPin = emotiBitVersionController.getAssignedPin(
      EmotiBitPinName::BATTERY_READ_PIN);
  buttonPin = emotiBitVersionController.getAssignedPin(
      EmotiBitPinName::EMOTIBIT_BUTTON);
  // TODO: Find a better way to swap pin assignments in different modes

  _emotiBitSystemConstants[(int)SystemConstants::EMOTIBIT_HIBERNATE_LEVEL] =
      emotiBitVersionController.getSystemConstant(
          SystemConstants::EMOTIBIT_HIBERNATE_LEVEL);
  _emotiBitSystemConstants[(int)SystemConstants::EMOTIBIT_HIBERNATE_PIN_MODE] =
      emotiBitVersionController.getSystemConstant(
          SystemConstants::EMOTIBIT_HIBERNATE_PIN_MODE);
  _emotiBitSystemConstants[(int)SystemConstants::LED_DRIVER_CURRENT] =
      emotiBitVersionController.getSystemConstant(
          SystemConstants::LED_DRIVER_CURRENT);
  // Setup switch
  ...
  adcRes =
      emotiBitVersionController.getMathConstant(MathConstants::ADC_MAX_VALUE);
  _vcc = emotiBitVersionController.getMathConstant(MathConstants::VCC);
  // edrAmplification =
  // emotiBitVersionController.getMathConstant(MathConstants::EDR_AMPLIFICATION);
  // edaFeedbackAmpR =
  // emotiBitVersionController.getMathConstant(MathConstants::EDA_FEEDBACK_R);
  ...
  while (Serial.available()) {
    ...
    if (input == 'A') {
      ...
      if (!adcCorrection.begin(adcCorrectionValues._gainCorrection,
                               adcCorrectionValues._offsetCorrection,
                               adcCorrectionValues.valid)) {
        Serial.println("Exiting ADC Correction.");
        delay(3000);
        break;
      }
      if (adcCorrectionValues.valid) {
        adcCorrection.echoResults(adcCorrectionValues._gainCorrection,
                                  adcCorrectionValues._offsetCorrection);
      }
#endif
    } else {
  ...
  dataDoubleBuffers[(uint8_t)EmotiBit::DataType::BATTERY_VOLTAGE] =
      &batteryVoltage;
  dataDoubleBuffers[(uint8_t)EmotiBit::DataType::BATTERY_PERCENT] =
      &batteryPercent;
  ...
  samplingRates.accelerometer =
      (float)BASE_SAMPLING_FREQ / (float)IMU_SAMPLING_DIV;
  samplingRates.gyroscope = (float)BASE_SAMPLING_FREQ / (float)IMU_SAMPLING_DIV;
  samplingRates.magnetometer =
      (float)BASE_SAMPLING_FREQ / (float)IMU_SAMPLING_DIV;
  samplingRates.eda = (float)BASE_SAMPLING_FREQ / (float)EDA_SAMPLING_DIV;
  samplingRates.humidity =
      (float)BASE_SAMPLING_FREQ / (float)TEMPERATURE_0_SAMPLING_DIV / 2.f;
  samplingRates.temperature =
      (float)BASE_SAMPLING_FREQ / (float)TEMPERATURE_0_SAMPLING_DIV / 2.f;
  samplingRates.temperature_1 =
      (float)BASE_SAMPLING_FREQ / (float)TEMPERATURE_1_SAMPLING_DIV;
  samplingRates.thermopile =
      (float)BASE_SAMPLING_FREQ / (float)THERMOPILE_SAMPLING_DIV;
  ...
  samplesAveraged.battery =
      (float)BASE_SAMPLING_FREQ / (float)BATTERY_SAMPLING_DIV / (0.2f);
  ...
  if (status) {
    ...
    if (_emotiBitSystemConstants[(int)SystemConstants::LED_DRIVER_CURRENT] >
        0) {
      led.setCurrent(
          _emotiBitSystemConstants[(int)SystemConstants::LED_DRIVER_CURRENT]);
    }
    ...
    if (testingMode == TestingMode::FACTORY_TEST) {
      EmotiBitFactoryTest::updateOutputString(
          factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::LED_CONTROLLER,
          EmotiBitFactoryTest::TypeTag::TEST_PASS);
    }
  ...
  // Initialize sensor
  while (ppgSensor.begin(*_EmotiBit_i2c) ==
         false) // reads the part number to confirm device
  {
    if (testingMode == TestingMode::FACTORY_TEST) {
      // FAIL
    }
    ...
    if (millis() - hibernateTimer > 2000) {
      EmotiBitFactoryTest::updateOutputString(
          factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::PPG_SENSOR,
          EmotiBitFactoryTest::TypeTag::TEST_FAIL);
      Serial.println(factoryTestSerialOutput);
      setupFailed("PPG");
    }
  ...
  ppgSensor.setup(ppgSettings.ledPowerLevel, ppgSettings.sampleAverage,
                  ppgSettings.ledMode, ppgSettings.sampleRate,
                  ppgSettings.pulseWidth, ppgSettings.adcRange);
  ppgSensor
      .enableDIETEMPRDY(); // Enable the temp ready interrupt. This is required
                           // to read die temperatures. Refer datasheet.
  ppgSensor.check();
  ...
  if (status) {
    ...
    if (testingMode == TestingMode::FACTORY_TEST) {
      // Add PASS
      EmotiBitFactoryTest::updateOutputString(
          factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::ACCEL_GYRO,
          EmotiBitFactoryTest::TypeTag::TEST_PASS);
      // ToDo: add the device ID
      String id = String(dev_id, HEX);
      EmotiBitFactoryTest::updateOutputString(
          factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::IMU_ID,
          id.c_str());
    }
    ...
    // Magnetometer
    BMI160.setRegister(BMI160_MAG_IF_0, BMM150_BASED_I2C_ADDR,
                       BMM150_BASED_I2C_MASK); // I2C MAG
    delay(BMI160_AUX_COM_DELAY);
    // initially load into setup mode to read trim values
    BMI160.setRegister(BMI160_MAG_IF_1, BMI160_MANUAL_MODE_EN_MSK,
                       BMI160_MANUAL_MODE_EN_MSK);
    delay(BMI160_AUX_COM_DELAY);
    ...
    // ADD_BMM_MEASURE delay(BMI160_AUX_COM_DELAY);

    // Following example at
    // https://github.com/BoschSensortec/BMI160_driver#auxiliary-fifo-data-parsing

    // Put the BMM150 in normal mode (may or may not be necessary if putting in
    // force mode later) BMI160.setRegister(BMM150_OPMODE_REG,
    // BMM150_DATA_RATE_10HZ | BMM150_NORMAL_MODE);
    // BMI160.setRegister(BMI160_MAG_IF_4, BMM150_DATA_RATE_10HZ |
    // BMM150_NORMAL_MODE);

    // BMI160.setRegister(BMI160_MAG_IF_4, BMM150_NORMAL_MODE);
    BMI160.reg_write_bits(BMI160_MAG_IF_4, BMM150_NORMAL_MODE,
                          BMM150_OP_MODE_BIT, BMM150_OP_MODE_LEN);
    BMI160.setRegister(BMI160_MAG_IF_3, BMM150_OP_MODE_ADDR);
    delay(BMI160_AUX_COM_DELAY);

    // Already done in setup
    ///* Set BMM150 repetitions for X/Y-Axis */
    // BMI160.setRegister(BMI160_MAG_IF_4, BMM150_LOWPOWER_REPXY); //Added for
    // BMM150 Support BMI160.setRegister(BMI160_MAG_IF_3, BMM150_XY_REP_REG);
    // //Added for BMM150 Support delay(BMI160_AUX_COM_DELAY);

    ///* Set BMM150 repetitions for Z-Axis */
    // BMI160.setRegister(BMI160_MAG_IF_4, BMM150_LOWPOWER_REPXY); //Added for
    // BMM150 Support BMI160.setRegister(BMI160_MAG_IF_3, BMM150_Z_REP_REG);
    // //Added for BMM150 Support delay(BMI160_AUX_COM_DELAY);

    // BMI160.setRegister(BMI160_MAG_IF_4, BMM150_DATA_RATE_25HZ);
    BMI160.reg_write_bits(BMI160_MAG_IF_4, BMM150_DATA_RATE_10HZ,
                          BMM150_DATA_RATE_BIT, BMM150_DATA_RATE_LEN);
    BMI160.setRegister(BMI160_MAG_IF_3, BMM150_OP_MODE_ADDR);
    delay(BMI160_AUX_COM_DELAY);

    // BMI160.setRegister(BMI160_MAG_IF_4, BMM150_FORCED_MODE);
    BMI160.reg_write_bits(BMI160_MAG_IF_4, BMM150_FORCED_MODE,
                          BMM150_OP_MODE_BIT, BMM150_OP_MODE_LEN);
    BMI160.setRegister(BMI160_MAG_IF_3, BMM150_OP_MODE_ADDR);
    delay(BMI160_AUX_COM_DELAY);
    ...
    BMI160.setRegister(BMI160_MAG_IF_1, BMI160_DISABLE,
                       BMI160_MANUAL_MODE_EN_MSK);

    // Set the burst length
    BMI160.setRegister(BMI160_MAG_IF_1, BMI160_AUX_READ_BURST_MSK,
                       BMI160_AUX_READ_BURST_MSK); // MAG data mode 8 byte burst
    delay(BMI160_AUX_COM_DELAY);
  ...
  if ((int)_hwVersion ==
      (int)EmotiBitVersionController::EmotiBitVersion::V03B) {
  ...
  if ((int)_hwVersion <=
    ...
    // moved the macro definition from EdaCorrection to
    // EmotiBitVersionController
    ...
    if (status) {
      if (testingMode == TestingMode::FACTORY_TEST) {
        // Add PASS
        EmotiBitFactoryTest::updateOutputString(
            factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::TEMP_SENSOR,
            EmotiBitFactoryTest::TypeTag::TEST_PASS);
      }
      ...
      emotibitDeviceId = String(tempHumiditySensor.sernum_a) + "-" +
                         String(tempHumiditySensor.sernum_b);
      // Serial.print("\n");
      Serial.print("\tModel: ");
  ...
  if (emotiBitSku.equals(EmotiBitVariants::EMOTIBIT_SKU_MD)) {
    ...
    MLX90632::status returnError; // Required as a parameter for begin()
                                  // function in the MLX library
    status = thermopile.begin(deviceAddress.MLX, *_EmotiBit_i2c, returnError);
    if (status) {
      if (testingMode == TestingMode::FACTORY_TEST) {
        EmotiBitFactoryTest::updateOutputString(
            factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::THERMOPILE,
            EmotiBitFactoryTest::TypeTag::TEST_PASS);
      }
      Serial.println("Success");
  ...
  if (!adcCorrectionValues.valid) {
    // Instantiate the ADC Correction class to read data from the AT-Winc flash
    // to calculate the correction values
    AdcCorrection adcCorrection(AdcCorrection::AdcCorrectionRigVersion::UNKNOWN,
                                adcCorrectionValues._gainCorrection,
                                adcCorrectionValues._offsetCorrection,
                                adcCorrectionValues.valid,
                                adcCorrectionValues._isrOffsetCorr);
    if (adcCorrection.atwincAdcDataCorruptionTest !=
            AdcCorrection::Status::FAILURE &&
        adcCorrection.atwincAdcMetaDataCorruptionTest !=
            AdcCorrection::Status::FAILURE) {
      ...
      analogReadCorrection(adcCorrectionValues._offsetCorrection,
                           adcCorrectionValues._gainCorrection);
    }
  }
  ...
  // Setup EDA
  Serial.println("\nInitializing EDA... ");
  if (emotibitEda.setup(
          _hwVersion, _samplingRates.eda / ((float)_samplesAveraged.eda), &eda,
          &edl, &edr, _EmotiBit_i2c, &edlBuffer, &edrBuffer)) {
    EmotiBitFactoryTest::updateOutputString(
        factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::ADC_INIT,
        EmotiBitFactoryTest::TypeTag::TEST_PASS);
    Serial.println("Completed");
  } else {
    EmotiBitFactoryTest::updateOutputString(
  ...
  // edaCrossoverFilterFreq =
  // emotiBitVersionController.getMathConstant(MathConstants::EDA_CROSSOVER_FILTER_FREQ);
  /*
  if (edaCrossoverFilterFreq > 0)// valid assignment of constant
  {
          _edlDigFiltAlpha = pow(M_E, -2.f * PI * edaCrossoverFilterFreq /
  (_samplingRates.eda / _samplesAveraged.eda));
  }*/
  /*
  ...
  */
  ...
  if (status) {
    EmotiBitFactoryTest::updateOutputString(
        factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::SD_CARD,
        EmotiBitFactoryTest::TypeTag::TEST_PASS);
  ...
  led.send();
  uint16_t attemptDelay = 20000; // in mS. ESP32 has been observed to take >10
                                 // seconds to resolve an enterprise connection
  uint8_t maxAttemptsPerCred = 1;
  uint32_t timeout = attemptDelay * maxAttemptsPerCred *
                     _emotiBitWiFi.getNumCredentials() *
                     2; // Try cycling through all credentials at least 2x
                        // before giving up and trying a restart
  ...
  if (testingMode == TestingMode::FACTORY_TEST) {
    ...
    EmotiBitFactoryTest::updateOutputString(
        factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::WIFI,
        EmotiBitFactoryTest::TypeTag::TEST_PASS);
  }
  ...
  typeTags[(uint8_t)EmotiBit::DataType::PPG_INFRARED] =
      EmotiBitPacket::TypeTag::PPG_INFRARED;
  typeTags[(uint8_t)EmotiBit::DataType::PPG_RED] =
      EmotiBitPacket::TypeTag::PPG_RED;
  typeTags[(uint8_t)EmotiBit::DataType::PPG_GREEN] =
      EmotiBitPacket::TypeTag::PPG_GREEN;
  typeTags[(uint8_t)EmotiBit::DataType::TEMPERATURE_0] =
      EmotiBitPacket::TypeTag::TEMPERATURE_0;
  typeTags[(uint8_t)EmotiBit::DataType::TEMPERATURE_1] =
      EmotiBitPacket::TypeTag::TEMPERATURE_1;
  typeTags[(uint8_t)EmotiBit::DataType::THERMOPILE] =
      EmotiBitPacket::TypeTag::THERMOPILE;
  typeTags[(uint8_t)EmotiBit::DataType::HUMIDITY_0] =
      EmotiBitPacket::TypeTag::HUMIDITY_0;
  typeTags[(uint8_t)EmotiBit::DataType::ACCELEROMETER_X] =
      EmotiBitPacket::TypeTag::ACCELEROMETER_X;
  typeTags[(uint8_t)EmotiBit::DataType::ACCELEROMETER_Y] =
      EmotiBitPacket::TypeTag::ACCELEROMETER_Y;
  typeTags[(uint8_t)EmotiBit::DataType::ACCELEROMETER_Z] =
      EmotiBitPacket::TypeTag::ACCELEROMETER_Z;
  typeTags[(uint8_t)EmotiBit::DataType::GYROSCOPE_X] =
      EmotiBitPacket::TypeTag::GYROSCOPE_X;
  typeTags[(uint8_t)EmotiBit::DataType::GYROSCOPE_Y] =
      EmotiBitPacket::TypeTag::GYROSCOPE_Y;
  typeTags[(uint8_t)EmotiBit::DataType::GYROSCOPE_Z] =
      EmotiBitPacket::TypeTag::GYROSCOPE_Z;
  typeTags[(uint8_t)EmotiBit::DataType::MAGNETOMETER_X] =
      EmotiBitPacket::TypeTag::MAGNETOMETER_X;
  typeTags[(uint8_t)EmotiBit::DataType::MAGNETOMETER_Y] =
      EmotiBitPacket::TypeTag::MAGNETOMETER_Y;
  typeTags[(uint8_t)EmotiBit::DataType::MAGNETOMETER_Z] =
      EmotiBitPacket::TypeTag::MAGNETOMETER_Z;
  typeTags[(uint8_t)EmotiBit::DataType::BATTERY_VOLTAGE] =
      EmotiBitPacket::TypeTag::BATTERY_VOLTAGE;
  typeTags[(uint8_t)EmotiBit::DataType::BATTERY_PERCENT] =
      EmotiBitPacket::TypeTag::BATTERY_PERCENT;
  typeTags[(uint8_t)EmotiBit::DataType::DATA_CLIPPING] =
      EmotiBitPacket::TypeTag::DATA_CLIPPING;
  typeTags[(uint8_t)EmotiBit::DataType::DATA_OVERFLOW] =
      EmotiBitPacket::TypeTag::DATA_OVERFLOW;
  typeTags[(uint8_t)EmotiBit::DataType::DEBUG] =
      EmotiBitPacket::TypeTag::EMOTIBIT_DEBUG;
  ...
  _EmotiBit_i2c->setClock(
      i2cClkMain); // Set clock to 400KHz except when accessing Si7013
  EmotiBitFactoryTest::updateOutputString(
      factoryTestSerialOutput, EmotiBitFactoryTest::TypeTag::SETUP_COMPLETE,
      EmotiBitFactoryTest::TypeTag::TEST_PASS);
#ifdef ADAFRUIT_FEATHER_M0
  ...
  if (!_sendTestData) {
    ...
    // setup timer
    timer = timerBegin(
        0, 80, true); // timer ticks with APB timer, which runs at 80MHz. This
                      // setting makes the timer tick every 1uS
  ...
  Serial.println(
      "100Hz PPG activated. Ensure correct settings in "
      "ofxOscilloscopeSettings.xml are used to correctly visualize live data.");
#endif
  Serial.println("");
  ...
  if (_debugMode) {
    ...
    Serial.println(
        "Enter ? to know more about available options in DEBUG MODE");
    Serial.println(
        "**********************************************************");
  }
  // ToDo: implement logic to determine return val
...
void EmotiBit::setupFailed(const String failureMode, int buttonPin) {
  ...
  while (1) {
    // NOTE: The button press check doesn't work on EmotiBit v02 because DVDD is
    // not enabled
    ...
    // not using delay to keep the CPU acitve from serial pings from host
    // computer
...
bool EmotiBit::setupSdCard() {
  ...
  for (int i = 0; i < 3; i++) {
    Serial.print(i);
    Serial.print(",");
    // begin function initializes SPI at MAX speed by deafult. check
    // /src/SpiDriver/SdSpiDriver.h for details on the constructor
#if defined ARDUINO_FEATHER_ESP32
    if (SD.begin(EmotiBitVersionController::SD_CARD_CHIP_SEL_PIN, SPI,
                 10000000)) // 10MHz works with 40MHz CPU, 20Mhz does NOT
#else
  ...
  if (!loadConfigFile(_configFilename)) {
    ...
    // ToDo: verify if we need a separate case for FACTORY_TEST. We should
    // always have a config file, since FACTORY TEST is a controlled environment
...
bool EmotiBit::addPacket(uint32_t timestamp, const String typeTag, float *data,
                         size_t dataLen, uint8_t precision,
                         bool printToSerial) {
  ...
  if (dataLen > 0) {
    ...
    header = EmotiBitPacket::createHeader(
        typeTag, timestamp, _outDataPacketCounter++, dataLen, protocolVersion);
    ...
    if (typeTag.equals(typeTags[(uint8_t)EmotiBit::DataType::DATA_CLIPPING]) ||
        typeTag.equals(typeTags[(uint8_t)EmotiBit::DataType::DATA_OVERFLOW])) {
      // Handle clippping and overflow as a special case
      for (uint8_t i = 0; i < (uint8_t)EmotiBit::DataType::length; i++) {
        // Add all the clipping/overflow events across all the buffers to the
        // packet
        if (i != (uint8_t)EmotiBit::DataType::DATA_CLIPPING &&
            i != (uint8_t)EmotiBit::DataType::DATA_OVERFLOW) {
          // Skip clipping & overflow types
          size_t count = 0;
          if (typeTag.equals(
                  typeTags[(uint8_t)EmotiBit::DataType::DATA_CLIPPING])) {
            count = dataDoubleBuffers[i]->getClippedCount(
                DoubleBufferFloat::BufferSelector::OUT);
          }
          if (typeTag.equals(
                  typeTags[(uint8_t)EmotiBit::DataType::DATA_OVERFLOW])) {
            count = dataDoubleBuffers[i]->getOverflowCount(
                DoubleBufferFloat::BufferSelector::OUT);
          }
          if (DC_DO_V2) {
            if (count > 0) {
              String temp = EmotiBitPacket::PAYLOAD_DELIMITER +
                            String(typeTags[i]) +
                            EmotiBitPacket::PAYLOAD_DELIMITER + String(count);
              if (_outDataPackets.length() + temp.length() <
                  OUT_MESSAGE_RESERVE_SIZE - 1) {
                ...
                Serial.print("ERROR: _outDataPackets exceeded "
                             "OUT_MESSAGE_RESERVE_SIZE ");
                Serial.println("[" + String(_outDataPackets.length()) + "+" +
                               String(temp.length()) + "/" +
                               String(OUT_MESSAGE_RESERVE_SIZE) + "]");
              }
            }
          } else {
            for (size_t n = 0; n < count; n++) {
              String temp =
                  EmotiBitPacket::PAYLOAD_DELIMITER + String(typeTags[i]);
              if (_outDataPackets.length() + temp.length() <
                  OUT_MESSAGE_RESERVE_SIZE - 1) {
                _outDataPackets += temp;
                if (printToSerial) {
      ...
      for (uint16_t d = 0; d < dataLen; d++) {
        String temp =
            EmotiBitPacket::PAYLOAD_DELIMITER + String(data[d], int(precision));
        if (_outDataPackets.length() + temp.length() <
            OUT_MESSAGE_RESERVE_SIZE - 1) {
          _outDataPackets += temp;
          if (printToSerial) {
          ...
          _outDataPackets += EmotiBitPacket::PAYLOAD_TRUNCATED;
          Serial.print(
              "ERROR: _outDataPackets exceeded OUT_MESSAGE_RESERVE_SIZE ");
          Serial.println("[" + String(_outDataPackets.length()) + "+" +
                         String(temp.length()) + "/" +
                         String(OUT_MESSAGE_RESERVE_SIZE + "]"));
          break;
...
bool EmotiBit::addPacket(uint32_t timestamp, EmotiBit::DataType t, float *data,
                         size_t dataLen, uint8_t precision) {
  return addPacket(timestamp, typeTags[(uint8_t)t], data, dataLen, precision,
                   _sendSerialData[(uint8_t)t]);
  // ToDo: implement logic to determine return val
  return true;
}
...
bool EmotiBit::addPacket(EmotiBit::DataType t) {
  ...
  if (t == EmotiBit::DataType::DATA_CLIPPING ||
      t == EmotiBit::DataType::DATA_OVERFLOW) {
    // Handle clippping and overflow as a special case
    dataLen = 0;
    timestamp = millis();
      ...
      if (i != (uint8_t)EmotiBit::DataType::DATA_CLIPPING &&
        ...
        if (DC_DO_V2) {
          if (count > 0) {
            ...
            // ToDo: consider whether dataLen should be counted per delimter
            // (+=2) or per DataType (+=1)
            dataLen += 2;
          }
        } else {
    ...
    if (dataLen > 0) {
      ...
      _newDataAvailable[(uint8_t)t] =
          true; // set new data is available in the outputBuffer
    }
  }
    ...
    if (dataStartChar > 0 || dataStartChar == EmotiBitPacket::NO_PACKET_DATA) {
      if (header.typeTag.equals(EmotiBitPacket::TypeTag::RECORD_BEGIN)) {
        ...
        String datetimeString =
            packet.substring(dataStartChar, packet.length());
        ...
        if (_dataFile) {
          ...
          Serial.println(" **");
          // ToDo: need to communicate back to Visualizer if we were able to
          // open a file
        } else {
      ...
      } else if (header.typeTag.equals(
                     EmotiBitPacket::TypeTag::RECORD_END)) { // Recording end
        if (_dataFile) {
          _dataFile.close();
        }
        _sdWrite = false;
        Serial.println("** Recording End **");
                     ...
                     EmotiBitPacket::TypeTag::MODE_NORMAL_POWER)) {
        setPowerMode(EmotiBit::PowerMode::NORMAL_POWER);
      } else if (header.typeTag.equals(
                     EmotiBitPacket::TypeTag::MODE_LOW_POWER)) {
        setPowerMode(EmotiBit::PowerMode::LOW_POWER);
      } else if (header.typeTag.equals(
                     EmotiBitPacket::TypeTag::MODE_MAX_LOW_POWER)) {
        setPowerMode(EmotiBit::PowerMode::MAX_LOW_POWER);
      } else if (header.typeTag.equals(
                     EmotiBitPacket::TypeTag::MODE_WIRELESS_OFF)) {
        setPowerMode(EmotiBit::PowerMode::WIRELESS_OFF);
      } else if (header.typeTag.equals(
                     EmotiBitPacket::TypeTag::MODE_HIBERNATE)) {
        setPowerMode(EmotiBit::PowerMode::HIBERNATE);
      } else if (header.typeTag.equals(
                     EmotiBitPacket::TypeTag::EMOTIBIT_DISCONNECT)) {
        _emotiBitWiFi.disconnect();
      }

      // Create a packet that can be logged with incrementing EmotiBit packet
      // number
      controlPackets += EmotiBitPacket::createPacket(
          header.typeTag, packetNumber++,
          packet.substring(dataStartChar, packet.length()), header.dataLength);
    }
  }
}
...
void EmotiBit::updateButtonPress() {
  ...
  if (buttonPressed) {
    ...
    if (millis() - buttonPressedTimer > minLongButtonPress) {
      ...
      EmotiBitPacket::createPacket(EmotiBitPacket::TypeTag::BUTTON_PRESS_LONG,
                                   _outDataPacketCounter++, "", 0);
      _outDataPackets += "\n";
      if (testingMode == TestingMode::FACTORY_TEST) {
        EmotiBitFactoryTest::sendMessage(
            EmotiBitPacket::TypeTag::BUTTON_PRESS_LONG);
      }
      (onLongPressCallback());
    }
  } else {
    if (buttonPreviouslyPressed) // Make sure button was actually pressed (not
                                 // just a loop lag)
    {
      if (millis() - buttonPressedTimer > minShortButtonPress &&
          millis() - buttonPressedTimer < minLongButtonPress) {
        Serial.print("onShortPress: ");
        Serial.println(millis() - buttonPressedTimer);
        EmotiBitPacket::createPacket(
            EmotiBitPacket::TypeTag::BUTTON_PRESS_SHORT,
            _outDataPacketCounter++, "", 0);
        _outDataPackets += "\n";
        if (testingMode == TestingMode::FACTORY_TEST) {
          EmotiBitFactoryTest::sendMessage(
    ...
    buttonPressedTimer =
        millis(); // reset the timer until the button is pressed
  }
}
...
uint8_t EmotiBit::update() {
  ...
  // NOTE:: An older firmware, v1.2.86 needs to be used to write EDA correction
  // values to V3 (OTP) and below. Newer FW version can read both NVMs (EEPROM
  // and OTP), but only have write ability for EEPROM
  ...
  size_t dataAvailable =
      readData(EmotiBit::DataType::BATTERY_PERCENT, &battPct, 1);
  if (dataAvailable > 0 && !_debugMode) {
    // Smooth the heck out of the battPct before we use it to call sleep.
    static DigitalFilter battSleepFilt = DigitalFilter(
        DigitalFilter::FilterType::IIR_LOWPASS,
        ((float)BASE_SAMPLING_FREQ) / ((float)BATTERY_SAMPLING_DIV) /
            ((float)batteryPercentBuffer.size()),
        0.2f);
...
int8_t EmotiBit::updatePPGData() {
  ...
  while (ppgSensor.available()) {
    status = status |
             pushData(EmotiBit::DataType::PPG_INFRARED, ppgSensor.getFIFOIR());
    status =
        status | pushData(EmotiBit::DataType::PPG_RED, ppgSensor.getFIFORed());
    status = status |
             pushData(EmotiBit::DataType::PPG_GREEN, ppgSensor.getFIFOGreen());
  ...
  if (tempHumiditySensor.getStatus() == Si7013::STATUS_IDLE) {
    if (tempHumiditySensor.isHumidityNew() == true) {
      ...
      if (humidityBuffer.size() >= _samplesAveraged.humidity) {
        status = status | pushData(EmotiBit::DataType::HUMIDITY_0,
                                   average(humidityBuffer),
                                   &(humidityBuffer.timestamp));
      ...
      if (temperatureBuffer.size() >= _samplesAveraged.temperature) {
        status = status | pushData(EmotiBit::DataType::TEMPERATURE_0,
                                   average(temperatureBuffer),
                                   &(temperatureBuffer.timestamp));
...
int8_t EmotiBit::updatePpgTempData() {
  ...
  static DigitalFilter filterTemp1(DigitalFilter::FilterType::IIR_LOWPASS,
                                   _samplingRates.temperature_1, 1);
  if (firstTime) {
...
int8_t EmotiBit::updateThermopileData() {
  ...
  int8_t status = 0;
  uint32_t timestamp;
  float AMB; // = 22991.97;  // raw data from thermopile needed for post
             // processing
  float
      Sto; // = -88.44;  // raw data from thermopile needed for post processing
  ...
  if (thermopileMode == MODE_STEP) {
    ...
    if (!thermopileBegun) {
      ...
      thermopile.getRawSensorValues(
          thermStatus, AMB,
          Sto); // Get the temperature of the object we're looking at in C
#ifdef DEBUG_THERM_UPDATE
...
int8_t EmotiBit::updateIMUData() {
  ...
  if (_imuFifoFrameLen > 0) {
    ...
    if (nBytes > MAX_FIFO_BYTES - _imuFifoFrameLen * 2) {
      ...
      for (uint8_t j = (uint8_t)EmotiBit::DataType::ACCELEROMETER_X;
           j <= (uint8_t)EmotiBit::DataType::MAGNETOMETER_Z; j++) {
        // Note: this for loop usage relies on all IMU data types being grouped
        // from AX to MZ
        dataDoubleBuffers[j]->incrOverflowCount(
            DoubleBufferFloat::BufferSelector::IN);
        imuBufferFull = true;
      }
    }
  ...
  for (uint16_t j = 0; j < nFrames; j++) {
    if (_imuFifoFrameLen > 0) {
      ...
      // Check for near-overflow of IMU double buffer and if so let data stay on
      // IMU FIFO
      bool bufferMaxed = false;
      for (uint8_t k = (uint8_t)EmotiBit::DataType::ACCELEROMETER_X;
           k <= (uint8_t)EmotiBit::DataType::MAGNETOMETER_Z; k++) {
        // Note: this for loop usage relies on all IMU data types being grouped
        // from AX to MZ
        if (dataDoubleBuffers[k]->size(DoubleBufferFloat::BufferSelector::IN) ==
            dataDoubleBuffers[k]->capacity(
                DoubleBufferFloat::BufferSelector::IN)) {
          bufferMaxed = true;
        }
      }
      if (bufferMaxed && !imuBufferFull) {
        // ToDo: Consider how to improve logic so that chip buffer is
        // incrementally emptied rather than dumped into DO events data is about
        // to overflow... leave it on the FIFO unless FIFO is also full
        break;
      }
      BMI160.getFIFOBytes(_imuBuffer, _imuFifoFrameLen);
      if (_imuFifoFrameLen == 20) {
        BMI160.extractMotion9(_imuBuffer, &ax, &ay, &az, &gx, &gy, &gz, &mx,
                              &my, &mz, &rh);
      } else {
      ...
      // BMI160.getMotion9Bosch(&ax, &ay, &az, &gx, &gy, &gz, &mx, &my, &mz,
      // &rh); BMI160.getMotion9Check(&ax, &ay, &az, &gx, &gy, &gz, &mx, &my,
      // &mz, &rh);

      Serial.println("getMotion9");
    }
    static DigitalFilter filterMx(DigitalFilter::FilterType::IIR_LOWPASS,
                                  _samplingRates.magnetometer, 2);
    static DigitalFilter filterMy(DigitalFilter::FilterType::IIR_LOWPASS,
                                  _samplingRates.magnetometer, 2);
    static DigitalFilter filterMz(DigitalFilter::FilterType::IIR_LOWPASS,
                                  _samplingRates.magnetometer, 2);
    ...
    pushData(EmotiBit::DataType::ACCELEROMETER_X, convertRawAcc(ax),
             &timestamp);
    pushData(EmotiBit::DataType::ACCELEROMETER_Y, convertRawAcc(ay),
             &timestamp);
    pushData(EmotiBit::DataType::ACCELEROMETER_Z, convertRawAcc(az),
             &timestamp);
    ...
    if (bmm150XYClipped) {
      dataDoubleBuffers[(uint8_t)EmotiBit::DataType::MAGNETOMETER_X]
          ->incrClippedCount(DoubleBufferFloat::BufferSelector::IN);
      dataDoubleBuffers[(uint8_t)EmotiBit::DataType::MAGNETOMETER_Y]
          ->incrClippedCount(DoubleBufferFloat::BufferSelector::IN);
      bmm150XYClipped = false;
    }
    if (bmm150ZHallClipped) {
      dataDoubleBuffers[(uint8_t)EmotiBit::DataType::MAGNETOMETER_Z]
          ->incrClippedCount(DoubleBufferFloat::BufferSelector::IN);
...
int8_t EmotiBit::checkIMUClipping(EmotiBit::DataType type, int16_t data,
                                  uint32_t timestamp) {
  if (data == 32767 || data == -32768) {
    // pushData(EmotiBit::DataType::DATA_CLIPPING, (int)type, &timestamp);
    dataDoubleBuffers[(uint8_t)type]->incrClippedCount(
        DoubleBufferFloat::BufferSelector::IN);
...
float EmotiBit::convertMagnetoX(int16_t mag_data_x, uint16_t data_rhall) {
  ...
  if ((mag_data_x != BMM150_XYAXES_FLIP_OVERFLOW_ADCVAL) && (data_rhall != 0) &&
      (bmm150TrimData.dig_xyz1 != 0)) {
    /*Processing compensation equations*/
    process_comp_x0 =
        (((float)bmm150TrimData.dig_xyz1) * 16384.0f / data_rhall);
    retval = (process_comp_x0 - 16384.0f);
    process_comp_x1 =
        ((float)bmm150TrimData.dig_xy2) * (retval * retval / 268435456.0f);
    process_comp_x2 =
        process_comp_x1 + retval * ((float)bmm150TrimData.dig_xy1) / 16384.0f;
    process_comp_x3 = ((float)bmm150TrimData.dig_x2) + 160.0f;
    process_comp_x4 =
        mag_data_x * ((process_comp_x2 + 256.0f) * process_comp_x3);
    retval = ((process_comp_x4 / 8192.0f) +
              (((float)bmm150TrimData.dig_x1) * 8.0f)) /
             16.0f;
  } else {
...
float EmotiBit::convertMagnetoY(int16_t mag_data_y, uint16_t data_rhall) {
  ...
  /* Overflow condition check */
  if ((mag_data_y != BMM150_XYAXES_FLIP_OVERFLOW_ADCVAL) && (data_rhall != 0) &&
      (bmm150TrimData.dig_xyz1 != 0)) {
    /*Processing compensation equations*/
    ...
    process_comp_y1 =
        ((float)bmm150TrimData.dig_xy2) * (retval * retval / 268435456.0f);
    process_comp_y2 =
        process_comp_y1 + retval * ((float)bmm150TrimData.dig_xy1) / 16384.0f;
    process_comp_y3 = ((float)bmm150TrimData.dig_y2) + 160.0f;
    process_comp_y4 =
        mag_data_y * (((process_comp_y2) + 256.0f) * process_comp_y3);
    retval = ((process_comp_y4 / 8192.0f) +
              (((float)bmm150TrimData.dig_y1) * 8.0f)) /
             16.0f;
  } else {
    /* overflow, set output to 0.0f */
    retval = BMM150_OVERFLOW_OUTPUT_FLOAT;
...
float EmotiBit::convertMagnetoZ(int16_t mag_data_z, uint16_t data_rhall) {
  ...
  if ((mag_data_z != BMM150_ZAXIS_HALL_OVERFLOW_ADCVAL) &&
    ...
    process_comp_z3 =
        ((float)bmm150TrimData.dig_z1) * ((float)data_rhall) / 32768.0f;
...
int8_t EmotiBit::pushData(EmotiBit::DataType type, float data,
                          uint32_t *timestamp) {
  // Serial.print("TypeTag: "); Serial.println(typeTags[(uint8_t)type]);
  if ((uint8_t)type < (uint8_t)EmotiBit::DataType::length) {
    uint8_t status =
        dataDoubleBuffers[(uint8_t)type]->push_back(data, timestamp);

    if (status & BufferFloat::ERROR_BUFFER_OVERFLOW ==
                     BufferFloat::ERROR_BUFFER_OVERFLOW) {
      // NOTE: DATA_OVERFLOW count is now stored in DoubleBufferFloat without a
      // separate buffer

      // store the buffer overflow type and time
      // dataDoubleBuffers[(uint8_t)EmotiBit::DataType::DATA_OVERFLOW]->push_back((uint8_t)type,
      // timestamp);
...
bool EmotiBit::processThermopileData() {
  ...
  static const int samplingInterval =
      1000000 / (_samplingRates.thermopile * _samplesAveraged.thermopile);
  static const int minSwapTime = max(500, min(samplingInterval / 10, 10000));

  // Serial.println("window: " + String(samplingInterval - (micros() -
  // _thermReadFinishedTime)));
  ...
  if (sizeAMB !=
      sizeSto) // interrupt hit between therm0AMB.getdata and therm0Sto.getdata
  {
    ...
    Serial.println("window: " +
                   String(samplingInterval - (waitEnd - readFinishedTime)));
    Serial.println("swap: " + String(swapEnd - swapStart));
    ...
    therm0AMB.incrOverflowCount(DoubleBufferFloat::BufferSelector::OUT,
                                mismatch);
    therm0Sto.incrOverflowCount(DoubleBufferFloat::BufferSelector::OUT,
                                mismatch);
  }
  ...
  for (uint8_t i = 0; i < n; i++) {
    ...
    if (dataAMB[i] == -2 && dataSto[i] == -2) {
      ...
      return true;
      // return
      // dataDoubleBuffers[(uint8_t)EmotiBit::DataType::THERMOPILE]->getData(data,
      // timestamp);
    }
    float objectTemp;
    ...
    if (createNan) {
      // if createNan is true generate nan values with a 0.1 probablity. This is
      // to test the MLX90632 library functionatlity that prevents one nan to
      // recursively create only nan values.
    ...
    if (isnan(objectTemp)) {
      ...
      debugPacket +=
          EmotiBitPacket::createPacket(EmotiBitPacket::TypeTag::EMOTIBIT_DEBUG,
                                       _outDataPacketCounter++, payloadAMB, 1);
      debugPacket +=
          EmotiBitPacket::createPacket(EmotiBitPacket::TypeTag::EMOTIBIT_DEBUG,
                                       _outDataPacketCounter++, payloadSto, 1);
  ...
  dataDoubleBuffers[(uint8_t)EmotiBit::DataType::THERMOPILE]->incrOverflowCount(
      DoubleBufferFloat::BufferSelector::IN,
      max(therm0AMB.getOverflowCount(DoubleBufferFloat::BufferSelector::OUT),
...
int8_t EmotiBit::updateBatteryData() {
  int8_t status;
  static DigitalFilter filterBattVolt(
      DigitalFilter::FilterType::IIR_LOWPASS,
      ((BASE_SAMPLING_FREQ) / (BATTERY_SAMPLING_DIV)) /
          (_samplesAveraged.battery),
      0.01);
...
int8_t EmotiBit::getBatteryPercent(float bv) {
  ...
  const float V0 = 3.4f;
  const float FACTOR_V100_V0 =
      1 / (V100 - V0) * 100.f; // Precalculate multiplier to save CPU
  if (bv > V100) {
    result = 100;
  } else if (bv > V0) {
...
bool EmotiBit::printConfigInfo(File &file, const String &datetimeString) {
...

  {
    ...
    // ToDo: Use EmotiBitPacket::TypeTag rather than fallable constants to set
    // typeTags
  ...
  if ((int)_hwVersion < (int)EmotiBitVersionController::EmotiBitVersion::V04A) {
    {
      ...
      infos[i]["nominal_srate"] =
          _samplingRates.humidity / _samplesAveraged.humidity;
      infos[i]["channel_format"] = "float";
...

    {
      ...
      infos[i]["channel_count"] = 1;
      infos[i]["nominal_srate"] =
          _samplingRates.temperature / _samplesAveraged.temperature;
      infos[i]["channel_format"] = "float";
...

  {
    ...
    infos[i]["channel_count"] = 1;
    infos[i]["nominal_srate"] =
        _samplingRates.temperature_1 / _samplesAveraged.temperature_1;
    infos[i]["channel_format"] = "float";
    infos[i]["units"] = "degrees celcius";
...

  {
    ...
    if (thermopileMode == MODE_CONTINUOUS) {
      ...
      infos[i]["nominal_srate"] =
          _samplingRates.thermopile / _samplesAveraged.thermopile;
    }
    infos[i]["channel_format"] = "float";
    infos[i]["units"] = "degrees celcius";
    setups[i] = infos[i].createNestedObject("setup");
...

  {
    ...
    infos[i]["channel_count"] = 3;
    infos[i]["nominal_srate"] =
        ppgSettings.sampleRate / ppgSettings.sampleAverage;
    infos[i]["channel_format"] = "float";
...
void EmotiBit::readSensors() {
  ...
  if (_debugMode) {
    readSensorsIntervalMin =
        min(readSensorsIntervalMin, (uint32_t)(micros() - readSensorsBegin));
    readSensorsIntervalMax =
        max(readSensorsIntervalMax, (uint32_t)(micros() - readSensorsBegin));
  }
  ...
  if (_debugMode) {
    readSensorsDurationMax.nvm = max(readSensorsDurationMax.nvm,
                                     (uint32_t)(micros() - read1SensorBegin));
    read1SensorBegin = micros();
  }

  // Battery (all analog reads must be in the ISR)
  // TODO: use the stored/averaged Battery value instead of calling
  // readBatteryPercent again
  if (acquireData.battery) {
    ...
    if (_debugMode) {
      readSensorsDurationMax.battery =
          max(readSensorsDurationMax.battery,
              (uint32_t)(micros() - read1SensorBegin));
      read1SensorBegin = micros();
    }
  }
  ...
  if (dummyIsrWithDelay) {
    ...
    if (dummyCounter == DUMMY_ISR_DIV) {
      ...
      for (uint8_t t = (uint8_t)DataType::PPG_INFRARED;
           t < (uint8_t)DataType::BATTERY_VOLTAGE; t++) {
    ...
    if (acquireData.eda) {
      ...
      if (_debugMode) {
        readSensorsDurationMax.eda =
            max(readSensorsDurationMax.eda,
                (uint32_t)(micros() - read1SensorBegin));
        read1SensorBegin = micros();
      }
    }
    ...
    if (chipBegun.SI7013 && acquireData.tempHumidity) {
      ...
      if (temperatureCounter == TEMPERATURE_0_SAMPLING_DIV) {
        ...
        // I.e. you must call updateTempHumidityData() 2x with a sufficient
        // measurement delay between calls to sample both temperature/humidity
        // and the thermistor
      ...
      if (_debugMode) {
        readSensorsDurationMax.tempHumidity =
            max(readSensorsDurationMax.tempHumidity,
                (uint32_t)(micros() - read1SensorBegin));
        read1SensorBegin = micros();
      }
    }
    ...
    if (chipBegun.MAX30101 && acquireData.tempPpg) {
      ...
      if (bottomTempCounter == TEMPERATURE_1_SAMPLING_DIV) {
        // we can add a comditional someday, when we have more than one sensor
        // providing bottom temp
      ...
      if (_debugMode) {
        readSensorsDurationMax.tempPpg =
            max(readSensorsDurationMax.tempPpg,
                (uint32_t)(micros() - read1SensorBegin));
        read1SensorBegin = micros();
      }
    }
    ...
    if (chipBegun.MLX90632 && acquireData.thermopile) {
      ...
      if (_debugMode) {
        readSensorsDurationMax.thermopile =
            max(readSensorsDurationMax.thermopile,
                (uint32_t)(micros() - read1SensorBegin));
        read1SensorBegin = micros();
      }
    }
    ...
    if (chipBegun.MAX30101 && acquireData.ppg) {
      ...
      if (_debugMode) {
        readSensorsDurationMax.ppg =
            max(readSensorsDurationMax.ppg,
                (uint32_t)(micros() - read1SensorBegin));
        read1SensorBegin = micros();
      }
    }
    ...
    if (chipBegun.BMI160 && chipBegun.BMM150 && acquireData.imu) {
      ...
      if (_debugMode) {
        readSensorsDurationMax.imu =
            max(readSensorsDurationMax.imu,
                (uint32_t)(micros() - read1SensorBegin));
        read1SensorBegin = micros();
      }
    }
    ...
    if (chipBegun.NCP5623) {
        ...
        if (testingMode != TestingMode::FACTORY_TEST) {
          if (buttonPressed) {
            ...
            if (_emotiBitWiFi.isConnected()) {
              ...
              if (_emotiBitWiFi.status(false) ==
                  WL_CONNECTED) // ToDo: assess if WiFi.status() is
            ...
            if (_sdWrite) {
              ...
              if (millis() - recordBlinkDuration >= 500) {
                led.setLED(uint8_t(EmotiBit::Led::RED),
                           !led.getLED(uint8_t(EmotiBit::Led::RED)));
                recordBlinkDuration = millis();
              }
            } else if (!_sdWrite &&
                       led.getLED(uint8_t(EmotiBit::Led::RED)) == true) {
              led.setLED(uint8_t(EmotiBit::Led::RED), false);
            }
          }
        }
      ...
      if (_debugMode) {
        readSensorsDurationMax.led =
            max(readSensorsDurationMax.led,
                (uint32_t)(micros() - read1SensorBegin));
        read1SensorBegin = micros();
      }
    }
  ...
  if (_debugMode) {
    readSensorsDurationMax.total = max(readSensorsDurationMax.total,
                                       (uint32_t)(micros() - readSensorsBegin));
  }
  if (acquireData.debug)
    pushData(EmotiBit::DataType::DEBUG,
             micros() - readSensorsBegin); // Add readSensors processing
                                           // duration to debugBuffer
...
void EmotiBit::processHeartRate() {
  ...
  static DigitalFilter heartRateFilter(
      DigitalFilter::FilterType::IIR_LOWPASS, 25,
      1); // heartbeat is signal with a variable "sampling rate". We are
          // choosing a hardcoded "sampling freq." of 25 to reduce noise.
  static DigitalFilter ppgSensorHighpass(
      DigitalFilter::FilterType::IIR_HIGHPASS, _samplingRates.ppg,
      1); // to remove respiration artifact. filter frequency selected
          // empirically
  ...
  /*
  static float respIirFiltData[20]; // buffer to hold IIR filtered data (removed
  respiration) static float iirFiltData[20]; // buffer to hold FIR filtered data
  (removed respiration) for(uint8_t i=0;i<20;i++)
  {
  ...
  for (uint16_t i = 0; i < dataSize; i++) {
    ...
    // the heart rate algorithm can be found in:
    // EmotiBit_MAX30101/src/heartRate.cpp
    ...
    if (isBeat) {
      ...
      addPacket(beatTime, EmotiBitPacket::TypeTag::INTER_BEAT_INTERVAL,
                &interBeatInterval, APERIODIC_DATA_LEN);
      addPacket(beatTime, EmotiBitPacket::TypeTag::HEART_RATE, &heartRate,
                APERIODIC_DATA_LEN);
  ...
  if (!overflowTestOn) {
    for (int16_t t = 0; t < (uint8_t)EmotiBit::DataType::length; t++) {
      if ((uint8_t)EmotiBit::DataType::EDA == t) {
        ...
        // In the future, processData() functions will be added to a linked list
        // for iteration
...
void EmotiBit::sleep(bool i2cSetupComplete) {
  ...
  // https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/system/sleep_modes.html#entering-deep-sleep
  ...
  // If version is known, avoid setting Hibernate pin as INPUT till emotibit is
  // powered OFF Setup all pins (digital and analog) in INPUT mode (default is
  // nothing)
  ...
  digitalWrite(
      EmotiBitVersionController::HIBERNATE_PIN,
      _emotiBitSystemConstants[(int)SystemConstants::EMOTIBIT_HIBERNATE_LEVEL]);
  delay(100);
  pinMode(EmotiBitVersionController::EMOTIBIT_I2C_CLK_PIN, OUTPUT);
  digitalWrite(EmotiBitVersionController::EMOTIBIT_I2C_CLK_PIN, LOW);
  ...
  pinMode(EmotiBitVersionController::HIBERNATE_PIN,
          _emotiBitSystemConstants[(
              int)SystemConstants::EMOTIBIT_HIBERNATE_PIN_MODE]);
...
bool EmotiBit::loadConfigFile(const String &filename) {
  ...
  for (size_t i = 0; i < configSize; i++) {
    String ssid = jsonDoc["WifiCredentials"][i]["ssid"] |
...
bool EmotiBit::writeSdCardMessage(const String &s) {
  // Break up the message in to bite-size chunks to avoid over running the UDP
  // or SD card write buffers UDP buffer seems to be about 1400 char. SD card
  // writes should be 512 char.
...
EmotiBit::PowerMode EmotiBit::getPowerMode() { return _powerMode; }
...
void EmotiBit::setPowerMode(PowerMode mode) {
  ...
  if (getPowerMode() == PowerMode::NORMAL_POWER) {
    ...
    if (_emotiBitWiFi.isOff()) {
      ...
      _emotiBitWiFi.begin(100, 1,
                          100); // This ToDo: create a async begin option
      Serial.print("Total WiFi.begin() = ");
      Serial.println(millis() - beginTime);
    }
#ifdef ADAFRUIT_FEATHER_M0
    // For ADAFRUIT_FEATHER_M0, lowPowerMode() is a good balance of performance
    // and battery
    WiFi.lowPowerMode();
    // For ESP32 the default WIFI_PS_MIN_MODEM is probably optimal
...
#endif
    // ToDo: for ESP32 There may be some value to explore WIFI_PS_MAX_MODEM
    // https://www.mischianti.org/2021/03/06/esp32-practical-power-saving-manage-wifi-and-cpu-1/
    modePacketInterval = LOW_POWER_MODE_PACKET_INTERVAL;
...
void EmotiBit::writeSerialData(EmotiBit::DataType t) {
  float *data;
  uint32_t timestamp;
  size_t dataAvailable = dataDoubleBuffers[(uint8_t)t]->getData(
      &data, &timestamp, false); // read data without swapping buffers
...
size_t EmotiBit::readData(EmotiBit::DataType t, float *data, size_t dataSize,
  ...
  if (testing) {
    if (_newDataAvailable[(
            uint8_t)t]) // if there is new data available on the outputBuffer
    {
  ...
  return bufferSize; // Return size of available buffer even if we're only able
                     // to copy some of it
}
...
size_t EmotiBit::readData(EmotiBit::DataType t, float **data,
  ...
  if ((uint8_t)t < (uint8_t)EmotiBit::DataType::length) {
    ...
    {
      _newDataAvailable[(uint8_t)t] = false;
      return dataDoubleBuffers[(uint8_t)t]->getData(
          data, &timestamp, false); // read data without swapping buffers
    }
  }
  return 0;
...
void EmotiBit::appendTestData(String &dataMessage, uint16_t &packetNumber) {
  for (uint8_t t = ((uint8_t)EmotiBit::DataType::DATA_CLIPPING) + 1;
       t < (uint8_t)DataType::length; t++) {
    String payload;
    int m = 5;
    int n = 2;
    for (int i = 0; i < m; i++) {
    ...
    dataMessage += EmotiBitPacket::createPacket(typeTags[t], packetNumber++,
                                                payload, m * n);
  }
}
...
bool EmotiBit::createModePacket(String &modePacket, uint16_t &packetNumber) {
  ...
  if (_sdWrite) {
    ...
    // ToDo: Consider if we want to send the sdCardFilename in a separate
    // key-value pair
    payload += ',';
  ...
  modePacket +=
      EmotiBitPacket::createPacket(EmotiBitPacket::TypeTag::EMOTIBIT_MODE,
                                   packetNumber++, payload, dataCount);
...
void EmotiBit::sendModePacket(String &sentModePacket, uint16_t &packetNumber) {
  ...
  // ToDo: This should probably be over TCP in response to specific messages
  // from Host (but will require writing TCP ingest)
  _emotiBitWiFi.sendData(
      sentModePacket);               // Send packet immediately to update host
  _outDataPackets += sentModePacket; // Add packet to slower data logging bucket
}

void EmotiBit::processDebugInputs(String &debugPackets,
                                  uint16_t &packetNumber) {
  if (Serial.available()) {
    uint8_t dataCount = 1;
    String payload;
    ...
    if (c == '?') {
      ...
      if (_serialData == DataType::length) {
        ...
        debugPackets += EmotiBitPacket::createPacket(
            EmotiBitPacket::TypeTag::EMOTIBIT_DEBUG, packetNumber++, payload,
            dataCount);
      } else {
...
#else
      Serial.print(
          "DEBUG_BUFFER must be defined to set acquireData.debug = true");
#endif
    } else if (c == 'b') {
...
void EmotiBit::processFactoryTestMessages() {
  if (Serial.available() > 0 &&
      Serial.read() == EmotiBitFactoryTest::MSG_START_CHAR) {
    ...
    if (msgTypeTag.equals(EmotiBitFactoryTest::TypeTag::LED_RED_ON)) {
      ...
      led.setLED((uint8_t)EmotiBit::Led::YELLOW, true);
    } else if (msgTypeTag.equals(
                   EmotiBitFactoryTest::TypeTag::LED_YELLOW_OFF)) {
      led.setLED((uint8_t)EmotiBit::Led::YELLOW, false);
      ...
      for (uint8_t i = 0; i < (uint8_t)EmotiBit::DataType::length; i++) {
        if (dataType.equals(typeTags[i])) {
          ...
          Serial.println(typeTags[i]);
                   ...
                   EmotiBitFactoryTest::TypeTag::EDA_CALIBRATION_VALUES)) {
      Serial.println(msg);
      if (emotibitEda.stageCalibStorage(&_emotibitNvmController, msg)) {
        ...
        if (emotibitEda.stageCalibLoad(&_emotibitNvmController)) {
          EmotiBitFactoryTest::sendMessage(
              EmotiBitFactoryTest::TypeTag::EDA_CALIBRATION_ACK);
        }
      }
    } else {
...
#elif defined ARDUINO_FEATHER_ESP32
void onTimer() { vTaskResume(EmotiBitDataAcquisition); }
#endif
...
void attachEmotiBit(EmotiBit *e) { myEmotiBit = e; }
#ifdef ADAFRUIT_FEATHER_M0
void ReadSensors() {
...
void EmotiBit::bufferOverflowTest(unsigned int maxTestDuration,
  ...
  while (totalDuration < maxTestDuration) {
    ...
    for (uint8_t d = 0; d < (uint8_t)DataType::length; d++) {
      ...
      Serial.print(", ");
      Serial.print(dataDoubleBuffers[(uint8_t)d]->size(
          DoubleBufferFloat::BufferSelector::IN));
      Serial.print(", ");
      Serial.print(dataDoubleBuffers[(uint8_t)d]->capacity(
          DoubleBufferFloat::BufferSelector::IN));
      Serial.print(", ");
      Serial.print(dataDoubleBuffers[(uint8_t)d]->getOverflowCount(
          DoubleBufferFloat::BufferSelector::IN));
      // if an overflow is detected on any stream, record the time of overflow
      if (timeFirstOverflow == 0 &&
          dataDoubleBuffers[(uint8_t)d]->getOverflowCount(
              DoubleBufferFloat::BufferSelector::IN))
