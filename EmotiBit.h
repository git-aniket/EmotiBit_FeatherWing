...
#elif defined(EMOTIBIT_PPG_1000HZ)
	struct PPGSettings
	{
		uint8_t ledPowerLevel = 0x2F; // Options: 0=Off to 255=50mA
...
/**
 * We want the sampling frequecy of PPG to be 1000Hz while keeping others
 * same as before. Since 150 is not a multiple of 1000, we take their LCM
 * i.e 9000, which is 60 times 150. This is used as a scaler, which ultimately
 * cancels out for all DIV's expect PPG_SAMPLING_DIV.
 */
#ifndef SAMPLING_FREQ_SCALER
#define SAMPLING_FREQ_SCALER 60
#endif
	/*
...
#if !defined(EMOTIBIT_PPG_1000HZ) && !defined(ARDUINO_FEATHER_ESP32)
