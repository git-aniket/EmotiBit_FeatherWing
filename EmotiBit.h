...
public:
  enum class TestingMode { NONE, CHRONIC, ACUTE, FACTORY_TEST, length };
  ...
  struct PPGSettings {
    ...
    uint16_t sampleRate =
        800; // Options: 50, 100, 200, 400, 800, 1000, 1600, 3200
  ...
  struct PPGSettings {
    ...
    uint16_t sampleRate =
        400; // Options: 50, 100, 200, 400, 800, 1000, 1600, 3200
  ...
  enum class Led { RED = 1, BLUE, YELLOW };

  enum class BattLevel {
    THRESHOLD_HIGH =
        50, // Set thresholds for changing led indication on board for battery
    // THRESHOLD_MED  = 15,
    THRESHOLD_LOW = 20,
  ...
  int thermopileFs =
      8; // *** NOTE *** changing this may wear out the Melexis flash
  uint8_t thermopileMode =
      MODE_STEP; // If changing to MODE_CONTINUOUS besure to adjust SAMPLING_DIV
                 // to match thermopile rate
  ...
  static const uint16_t MAX_SD_WRITE_LEN =
      512; // 512 is the size of the sdFat buffer
...
#define TIMER_PRESCALER_DIV 1024
  ...
  const uint32_t CPU_HZ =
      80000000; // 80MHz has been tested working to save battery life
#elif defined(ADAFRUIT_FEATHER_M0)
   ...
   * If a sampling rate can be set for a sensor, it is done so in the sensor
   * settigns. The Firmware reads data from the sensor
   * @BASE_SAMPLING_FREQ/{SENSOR}_SAMPLING_DIV Hz further, the recorded samples
   * may be avaraged, as set by EmotiBit::SamplesAveraged struct. So, net, the
   * sampling rate of the data channel is
   * BASE_SAMPLING_FREQ/{DATATYPE}_SAMPLING_DIV/SamplesAveraged.{DATATYPE}
   */
...
#define THERMOPILE_SAMPLING_DIV                                                \
  20 // TODO: This should change according to the rate set on the thermopile
     // begin function
...
#define DUMMY_ISR_DIV 10
  ...
  // #define THERMOPILE_SAMPLING_DIV 16	// TODO: This should change according to
  // the rate set on the thermopile begin function #define
  // TEMPERATURE_0_SAMPLING_DIV 4 #define BATTERY_SAMPLING_DIV 20
  ...
  struct AcquireData {
    ...
    bool debug = false;
    bool battery = false;
    bool heartRate = false; // Note: we may want to move this to a separarte
                            // flag someday, for controlling derivative signals
  ...
  bool addPacket(uint32_t timestamp, const String typeTag, float *data,
                 size_t dataLen, uint8_t precision = 0,
                 bool printToSerial = false);
  bool addPacket(uint32_t timestamp, EmotiBit::DataType t, float *data,
                 size_t dataLen, uint8_t precision = 4);
  ...
  void processData();
  void sendData();
  bool processThermopileData(); // placeholder until separate EmotiBitThermopile
  ...
  /**
   * Copies data buffer and timestamp of the specified DataType into the passed
   * parameters
   *
   * @param t an EmotiBit::DataType to read
   * @data a pre-allocated array of size dataSize to copy data into
   ...
   *		CAUTION: this pointer becomes stale after calling
   *EmotiBit::update()
   *
   * @param t an EmotiBit::DataType to read
  ...
  /**
   * Points to the data buffer and reads the timestamp of the specified type
   *into the passed parameters CAUTION: this pointer becomes stale after calling
   *EmotiBit::update()
   *
   * @param t an EmotiBit::DataType to read
   * @data returns with a pointer to the existing (double buffered) float[]
   ...
   * @param humanReadable Boolean to switch between commas and carriage returns
   * for human readability
   */
  void bufferOverflowTest(unsigned int maxTestDuration = 5000,
  ...
  /*!
          ...
          @param String firmwareVariant typically passes name of .ino file for
     traceability
          @return 0 if successful, otherwise error code [ToDo: Add setup error
     codes]
  */
  ...
  int8_t updateIMUData(); /**< Read any available IMU data from the sensor FIFO
                             into the inputBuffers */
  int8_t updatePPGData(); /**< Read any available PPG data from the sensor FIFO
                             into the inputBuffers */
  int8_t updateTempHumidityData(); /**< Read any available temperature and
                                      humidity data into the inputBuffers */
  int8_t
  updatePpgTempData(); /**< Read any available temp data from PPG into buffer*/
  int8_t updateThermopileData(); /**< Read Thermopile data into the buffers*/
  int8_t updateBatteryData();    /**< Reads battery voltage and passes it to
                                    different buffers for update */
  int8_t updateBatteryVoltageData(
      float battVolt); /**< updates battery voltage inputBuffer */
  int8_t updateBatteryPercentData(
      float battPercent); /**< updates battery percentage inputBuffer */
  ...
  size_t getData(
      DataType t, float **data,
      uint32_t *timestamp = nullptr); /**< Swap buffers and read out the data */
...
private:
  ...
  int8_t checkIMUClipping(EmotiBit::DataType type, int16_t data,
                          uint32_t timestamp);
  int8_t
  pushData(EmotiBit::DataType type, float data,
           uint32_t *timestamp = nullptr); // Deprecated. BUFFER_OVERFLOW count
                                           // built into DoubleBuffer now.
  ...
  float _gyroRange;          // supported values: 125, 250, 500, 1000, 2000
                             // (degrees/second)
  ...
  DoubleBufferFloat temp1 = DoubleBufferFloat(
      TEMP_BUFFER_SIZE); // To store the thermistor Object Temp
  DoubleBufferFloat therm0 = DoubleBufferFloat(
      TEMP_BUFFER_SIZE); // To store the thermistor Object Temp
  DoubleBufferFloat therm0AMB = DoubleBufferFloat(
      TEMP_BUFFER_SIZE); // To store the raw value AMB from the thermistor
  DoubleBufferFloat therm0Sto = DoubleBufferFloat(
      TEMP_BUFFER_SIZE); // To store the raw Value Sto from the thermistor
  DoubleBufferFloat humidity0 = DoubleBufferFloat(TEMP_BUFFER_SIZE);
  DoubleBufferFloat accelX = DoubleBufferFloat(
      IMU_BUFFER_SIZE); // Account for IMU_CHIP_BUFFER_SIZE when creating
  ...
  DoubleBufferFloat dataOverflow =
      DoubleBufferFloat(1); // ToDo: Refactor with nullptr checks for removal
  DoubleBufferFloat dataClipping =
      DoubleBufferFloat(1); // ToDo: Refactor with nullptr checks for removal
#ifdef DEBUG_BUFFER
  DoubleBufferFloat debugBuffer = DoubleBufferFloat(MAX_DATA_BUFFER_SIZE);
#else
  DoubleBufferFloat debugBuffer =
      DoubleBufferFloat(1); // ToDo: Refactor for nullptr checks
#endif

  DoubleBufferFloat *dataDoubleBuffers[(uint8_t)DataType::length];

  // Oversampling buffers
  // Single buffered arrays must only be accessed from ISR functions, not in the
  // main loop ToDo: add assignment for dynamic allocation;
